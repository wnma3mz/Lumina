...
class TestBasicLoad:
    def test_loads_host_and_port(self, tmp_path):
        cfg = Config.load(write_config(tmp_path, {**minimal_config(), "host": "0.0.0.0", "port": 9999}))
    ...
    def test_loads_mlx_memory_nested_block(self, tmp_path):
        data = minimal_config()
        ...
        assert cfg.provider.offload_embedding is False
        assert cfg.provider.offload_vision is True
        assert cfg.provider.offload_audio is False
    ...
    def test_system_prompts_loaded(self, tmp_path):
        data = {**minimal_config(), "system_prompts": {"chat": "You are helpful."}}
        cfg = Config.load(write_config(tmp_path, data))
    ...
    def test_ui_home_loaded(self, tmp_path):
        ...
        cfg = Config.load(write_config(tmp_path, data))
        assert cfg.ui.home.enabled_tabs == ["digest", "document", "image", "audio", "stats", "settings"]
...
class TestRequestHistoryConfig:
    ...
    def test_custom_values(self, tmp_path):
        data = {**minimal_config(), "request_history": {
            "enabled": False,
            "retention_days": 7,
            "max_total_mb": 100,
    ...
    def test_retention_days_clamped_to_zero(self, tmp_path):
        data = {**minimal_config(), "request_history": {"retention_days": -5}}
        cfg = Config.load(write_config(tmp_path, data))
        assert cfg.request_history.retention_days == 0

    def test_max_total_mb_clamped_to_one(self, tmp_path):
        data = {**minimal_config(), "request_history": {"max_total_mb": 0}}
        cfg = Config.load(write_config(tmp_path, data))
...
class TestRuntimeConfigMirrors:
    def test_digest_get_cfg_prefers_loaded_main_config(self, tmp_path):
        from lumina.config import get_config
        from lumina.services.digest.config import get_cfg

        data = {
            **minimal_config(),
            "digest": {"enabled": True, "active_watch_dirs": ["/tmp/a", "/tmp/b"]},
        }
        ...
        digest_cfg = get_cfg()
        assert digest_cfg is cfg.digest
        assert digest_cfg.active_watch_dirs == ["/tmp/a", "/tmp/b"]

    def test_request_history_get_cfg_prefers_loaded_main_config(self, tmp_path):
        from lumina.config import get_config
        from lumina.engine import request_history

        data = {
            **minimal_config(),
            "request_history": {"enabled": False, "retention_days": 3},
        }
        path = write_config(tmp_path, data)
        set_active_config_path(path)
        cfg = get_config(path)

        rh_cfg = request_history.get_cfg()
        assert rh_cfg is cfg.request_history
        assert rh_cfg.enabled is False
        assert rh_cfg.retention_days == 3

    def test_request_history_configure_updates_loaded_main_config(self, tmp_path):
        from lumina.config import get_config
        from lumina.engine import request_history
        ...
        request_history.configure(
            {"request_history": {"enabled": False, "retention_days": 5}},
            run_startup_cleanup=False,
        )

        assert cfg.request_history.enabled is False
        assert cfg.request_history.retention_days == 5
        request_history.shutdown()

    def test_transcriber_initial_prompt_prefers_main_config(self, tmp_path):
        from lumina.config import get_config

        data = {
            **minimal_config(),
            "audio": {"prompts": {"asr_zh": "主配置中文提示词", "asr_en": "main english prompt"}},
        }
        path = write_config(tmp_path, data)
        set_active_config_path(path)
        get_config(path)
        ...
        with patch.dict(
            ...
            from lumina.services.audio import transcriber as transcriber_mod

            transcriber_mod.set_asr_prompts("旧中文提示词", "old english prompt")
            assert transcriber_mod._make_initial_prompt("zh") == "主配置中文提示词"
            assert transcriber_mod._make_initial_prompt("en") == "main english prompt"
