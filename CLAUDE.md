...
- **`style.css` 是编译产物，禁止手动修改**：`style.css` 由 Tailwind CLI 从 `input.css` 编译生成，任何手动修改都会在下次编译时被覆盖。所有样式改动必须在 `input.css` 里进行，然后重新编译：`npx tailwindcss -i lumina/api/static/input.css -o lumina/api/static/style.css`，再提交两个文件。
- **`input.css` 是唯一样式源**：所有样式修改、新增原子类或自定义组件，必须在 `lumina/api/static/input.css` 中进行。
- **无运行时构建步骤**：`node_modules/`、`package.json`、`tailwind.config.js`、`package-lock.json` 在 `.gitignore` 中，不提交。`input.css` 已提交到 git，运行项目不需要编译环境。需要改样式时，先在项目根目录安装 Tailwind v3（**必须是 v3，`input.css` 用 v3 语法**）再编译：
  ```bash
  npm install --save-dev tailwindcss@3
  ./node_modules/.bin/tailwindcss -i lumina/api/static/input.css -o lumina/api/static/style.css
  ```
...
- **style.css 修改方式**：改 `input.css` → 重新编译 → 提交 `style.css`。不要直接编辑 `style.css`，编译会覆盖它。新增 tab 时需要在 `input.css` 里补齐六组选择器：panel hide、:checked show、nav-active 高亮、not-checked 非活跃、:hover、:hover:is(.dark *)。
