...
- **`style.css` 的两种维护路径**（二者择其一，不要混用）：
  - **路径 A（推荐）**：在 `input.css` 里改 → 用 `npx tailwindcss -i lumina/api/static/input.css -o lumina/api/static/style.css` 重新编译 → 提交两个文件。适用于需要扫描新 Tailwind 工具类的情况。
  - **路径 B（仅限例外）**：直接编辑 `style.css`，同时在 `input.css` 里写等效的纯 CSS（不带 `@apply` / Tailwind 前缀）。适用于 `input.css` 不依赖 Tailwind 扫描的情况，例如：CSS 选择器逻辑（tab 切换规则）、包含 `:hover:is(.dark *)` 等复合伪类的规则。改完必须保证两个文件语义完全一致，下次编译不会把 style.css 里的改动抹掉。
...
- **`@apply` 只能用在 `@layer` 块内**：`input.css` 中 `@layer components` / `@layer utilities` 之外的纯 CSS 选择器块不能使用带状态前缀的 `@apply`（如 `@apply hover:text-zinc-900` 或 `@apply dark:hover:text-white`）。Tailwind 编译这些前缀时依赖 JIT 上下文，在 `@layer` 外不会展开。正确做法：直接写 CSS 属性值（如 `color: rgb(24 24 27)`），`:hover:is(.dark *)` 复合伪类也直接写选择器。
...
- **style.css 修改方式**：见「前端开发准则 → 技术栈约定」中的路径 A / 路径 B 说明。核心原则：input.css 和 style.css 必须始终保持语义一致，不能让一边有而另一边没有某条规则（尤其是 tab 切换的选择器组：panel hide、:checked show、nav-active 高亮、not-checked 非活跃、hover、dark hover 六组必须同时在两个文件里完整出现）。
