...
- **`style.css` 当前是手动维护的**：理论上 `style.css` 应由 Tailwind CLI 编译生成，但由于 `tailwind.config.js`（含 `content` 扫描路径）已被 `.gitignore` 排除，**直接运行编译会产出残缺的 CSS**（只扫描默认路径，遗漏 `templates/` 里的 class）。因此 `style.css` 目前以手动方式维护，直接提交到 git。
- **改样式的正确做法**：直接编辑 `style.css`，同时在 `input.css` 里写等效内容保持同步（`input.css` 作为人类可读的注释源）。若将来要恢复完整编译链，需要先重建 `tailwind.config.js` 并配置正确的 `content` 路径。
- **`input.css` 的作用**：记录样式的语义意图（使用 `@apply`、`@layer` 等 Tailwind 语法），但不直接生效。浏览器读的是 `style.css`。
...
- **style.css 修改方式**：直接编辑 `style.css`，同时在 `input.css` 写等效内容保持同步。新增 tab 时需要在两个文件里同时补齐六组选择器：panel hide、:checked show、nav-active 高亮、not-checked 非活跃、:hover、:hover:is(.dark *)。
